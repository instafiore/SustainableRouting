...
def _run_solver(rules, solver_path, options, timeout, resultFilename = None):
    filename = tempfile.gettempdir() + os.path.sep + "pyspel_tmp_program_%s" % uuid.uuid4()
    with open(filename, "w+") as f:
        f.write(rules)
        f.close()

    isTmpResultFilename = False
    if not resultFilename:
        isTmpResultFilename = True
        resultFilename = tempfile.gettempdir() + os.path.sep + "pyspel_tmp_program_%s" % uuid.uuid4()
    ...
    solver = subprocess.Popen(commands, stdin=None, stdout=resultFile, stderr=subprocess.PIPE)
    killed = False
    exit_code = 1
    try:
    ...
    fRead = open(resultFilename, "r")
    stdout = fRead.read()
    fRead.close()

    if isTmpResultFilename:
        os.remove(resultFilename)

    return stdout, stderr.decode(), exit_code, killed
...
class Answer:
    ...
    def __init__(self, answer_set, costs, optimal, stats: AnswerStats = None):
        self.answer_set = answer_set
        self.costs = costs
        self.optimal = optimal
        self.stats: AnswerStats = stats

    def getStats(self) -> AnswerStats:
        return self.stats
    ...
    def process_asp_facts(cls, filename, atoms, solver_path=None):
        ...
        if res['Result'] == 'SATISFIABLE':
            ...
            if 'Value' in answer_set:
                answer = Answer(answer_set['Value'], costs, False, stats=AnswerStats(res))
                output = []
...
class SolverWrapper:
    ...
    def solve(self, problem, options=None, print_solver_output=False, timeout=None, resultFilename=None):
        self.killed = False
        if options is None:
            options = []
        ...
        (stdout, stderr, exit_code, killed) = _run_solver(str(problem), self._solver_path, options, timeout=timeout, resultFilename=resultFilename)
        self.killed = killed
        ...
        try:
            ...
            return Result(Result.JSON_ERROR)
        ...
        if res['Result'] == 'UNSATISFIABLE':
            ...
            for answer_set in res['Call'][0]['Witnesses']:
                if 'Value' in answer_set:
                    r.add_answer(Answer(answer_set['Value'], costs, False, stats=AnswerStats(res)))
            ...
            for answer_set in res['Call'][0]['Witnesses']:
                if 'Value' in answer_set:
                    r.add_answer(Answer(answer_set['Value'], costs, True, stats=AnswerStats(res)))
                if 'Costs' in answer_set:
                    for cost in answer_set['Costs']:
                        costs.append(cost)
                    costs = [].copy()
